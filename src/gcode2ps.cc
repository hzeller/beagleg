...
class Device {
 public:
  ...
  void MeasureLine(float min, float max, float size, bool show_metric,
    ...
    draw(false, 0, 0);
    draw(true, width, 0);  // full length
    draw(false, width, -size);
    draw(true, width, size);  // second tick.
 ...
 public:
  SegmentQueuePrinter(Device *device, const MachineControlConfig &config,
    ...
    if (opts_.output_js_vertices) {
      ...
      fprintf(stdout, "[%.3f, %.3f, %.3f], [%.3f, %.3f, %.3f]];\n", 0.0, 0.0,
              0.0, config_.move_range_mm[AXIS_X], config_.move_range_mm[AXIS_Y],
              config_.move_range_mm[AXIS_Z]);
    }
  }
  ...
  bool Enqueue(const LinearSegmentSteps &param) final {
    ...
    case ProcessingStep::Preprocess: {
      ...
      if (abs(param.steps[AXIS_X]) + abs(param.steps[AXIS_Y]) +
            abs(param.steps[AXIS_Z]) <=
          3)
      ...
      const float segment_len =
        sqrtf(dx_mm * dx_mm + dy_mm * dy_mm + dz_mm * dz_mm);
      ...
      const float segment_speed_factor =
        segment_len /
  ...
  void SetColorSegmentLength(float length) { color_segment_length_ = length; }
  ...
  bool isWithinMachineCube(const MotorsRegister &m) const {
    ...
    return inRange(m[AXIS_X], AXIS_X) && inRange(m[AXIS_Y], AXIS_Y) &&
    ...
    if (opts_.show_speeds) {
      ...
      for (int i = 0; i < segments; ++i) {
        ...
        if (is_valid_position) {
          int col_idx;
          if (v < min_color_range_)
            col_idx = 0;
          else if (v > max_color_range_)
            col_idx = 255;
          else {
...
static int usage(const char *progname, bool description = false) {
  ...
  fprintf(
    ...
    "\t-g<grid>          : Show grid on XY plane. Optional with 'in' unit "
    "suffix (default: mm).\n"
    "\t-C<caption>       : Title text shown on XY plane.\n"
    "[---- Rotation. Multiple can be applied in sequence ----]\n"
    "\t-R<roll>          : Roll: Rotate around axis pointing towards and "
    "through canvas\n"
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "a:A:c:C:De:g:GilMo:P:qrR:sS:t:T:V:w:Y:")) !=
    ...
    case 't': threshold_angle = (float)atof(optarg); break;
    case 'A': speed_tune_angle = (float)atof(optarg); break;
    case 'g':
      grid = (float)atof(optarg);
      if (strstr(optarg, "in") != nullptr) grid *= 25.4;
      break;
    case 'l': vis_options.show_laser_burn = true; break;
    case 'C': vis_options.show_title = optarg; break;
    case 'c': config_file = strdup(optarg); break;
    case 'D': show_dimensions = false; break;
    case 'M': show_machine_path = false; break;
    case 'G': show_gcode_path = false; break;
    case 'T': tool_diameter_mm = atof(optarg); break;
    case 's': vis_options.show_speeds = true; break;
    ...
    case 'w':
      ...
      break;
    case 'e': eye_distance = atof(optarg); break;
    case 'i': vis_options.show_ijk = !vis_options.show_ijk; break;
    case 'r': range_check = true; break;
    case 'a': animation_frames = atoi(optarg); break;
    case 'R':
  ...
  if (optind >= argc) return usage(argv[0], true);
  ...
  if (vis_options.show_speeds && !config_file) {
    fprintf(stderr,
            "FYI: Need machine-control config (-c) "
            "to show speeds (-s)\n");
  ...
  if (vis_options.show_laser_burn && !show_gcode_path) {
    ...
    fprintf(stderr,
            "FYI: Can only show laser color (-l) if GCode path is "
            "not disabled with -G\n");
  ...
  if (tool_diameter_mm >= 0 && !config_file) {
    fprintf(stderr,
            "FYI: Tool diameter (-T) only visualized "
