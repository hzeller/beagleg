...
static void Log_internal(int fd, const char *markup_start, const char *format,
                         va_list ap) {
  struct timeval now;
  ...
  parts[0].iov_len = asprintf((char **)&parts[0].iov_base, "%s[%s.%06ld]%s ",
                              markup_start, fmt_buf, now.tv_usec, markup_end_);
