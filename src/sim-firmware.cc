...
the right axis. Config: motor mapping motor_1: axis:x, motor_2:axis:y,
motor_3:axis:z
...
#.. as one-liner
plot "/tmp/foo.data" using 1:5 title "steps X" with lines ls 10, '' using 1:6
title "velocity X" with lines ls 11, '' using 1:7 title "accel X" axes x1y2 with
lines ls 12, '' using 1:8 title "steps Y" with lines ls 20, '' using 1:9 title
"velocity Y" with lines ls 21, '' using 1:10 title "accel Y" axes x1y2 with
lines ls 22

# Euclid space velocity and acceleration.
plot "/tmp/foo.data" using 1:3 title "velocity Euclid" with lines ls 1, '' using
1:4 title "accel Euclid" axes x1y2 with lines ls 2
...
/*
 * Due to the timer accuracy, velocity is quantized (sometimes, adjacent steps
 * have the exact velocity followed by a step in velocity) Calculating the
 * acceleration requires some smoothing moving average over a couple of
 * measurements. The minimum is 2 to look at two adjacent values.
 */
...
class SimFirmwareQueue::Averager {
 public:
  ...
  double GetAcceleration() {
    if (pos_ < 2) return 0;
    // We go back as far as the ringbuffer reaches. In the beginning, that is
    // not far.
...
static int sim_steps[MOTION_MOTOR_COUNT];  // we are only looking at the
                                           // defining axis steps.
...
bool SimFirmwareQueue::Enqueue(MotionSegment *segment) {
  ...
  const double euklid_factor =
    euclid(motor_speeds[X_MOTOR], motor_speeds[Y_MOTOR], motor_speeds[Z_MOTOR]);
  ...
  for (;;) {
    ...
    // Higher resolution delay if we had fractional counts. Used to better
    // calculate acceleration for display purposes.
    ...
    if (segment->jerk_start > 0) {
      ...
      if (is_first) {
        msg = "# accel.";
        fprintf(stderr,
                "SIM: Accel start _/ : accel-series-idx=%5u, "
      ...
      if (segment->loops_accel == 0) {
        fprintf(stderr,
                "SIM: Accel end   /‾ : accel-series-idx=%5u, "
                "accel-timer-cycles=%10.3f\n",
                segment->accel_series_index,
                1.0 * segment->hires_accel_cycles / (1 << DELAY_CYCLE_SHIFT));
      }
      ...
      if (is_first) {
        ...
        fprintf(stderr,
                "SIM: travel      -- :                               "
                "timer-cycles=%6u     (%d loops)\n",
                delay_loops, segment->loops_travel);
        is_first = false;
      }
      ...
      if (is_first) {
        msg = "# decel.";
        fprintf(stderr,
                "SIM: Decel start ‾\\ : accel-series-idx=%5u, "
      ...
      if (segment->loops_decel == 0) {
        fprintf(stderr,
                "SIM: Decel end   \\_ : accel-series-idx=%5u, "
                "decel-timer-cycles=%10.3f\n",
                segment->accel_series_index,
                1.0 * segment->hires_accel_cycles / (1 << DELAY_CYCLE_SHIFT));
      }
    } else {
    ...
    // motors].
    fprintf(out_, "%12.8f %10d %12.4f %12.4f      ", sim_time, delay_loops,
            euklid_factor * velocity, euklid_factor * acceleration);
    for (int i = 0; i < relevant_motors_; ++i) {
      fprintf(out_, "%5d %10.4f %12.4f ", sim_steps[i],
              motor_speeds[i] * velocity, motor_speeds[i] * acceleration);
    }
    fprintf(out_, "%s\n", msg);
  }
  return true;
...
SimFirmwareQueue::SimFirmwareQueue(FILE *out, int relevant_motors)
  ...
  // Total time; speed; acceleration; delay_loops. [steps walked for all
  // motors].
  printf("%12s %10s %12s %12s      ", "time", "timer-loop", "Euclid-speed",
         "Euclid-accel");
  for (int i = 0; i < relevant_motors_; ++i) {
    fprintf(out_, "%4s%d %9s%d %11s%d ", "s", i, "v", i, "a", i);
  }
  fprintf(out_, "\n");
}

SimFirmwareQueue::~SimFirmwareQueue() { delete averager_; }
