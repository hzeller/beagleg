...
You can easily test if the module it's available by running:
    ...
    if sudo lsmod | grep "uio_pruss" &> /dev/null ; then echo "the kernel is BeagleG ready!"; fi
...
if the uio_pruss module is not available, you will need to
[change your kernel version](http://elinux.org/BeagleBoardDebian#Install_Latest_Kernel_Image) to a bone prefix one.
